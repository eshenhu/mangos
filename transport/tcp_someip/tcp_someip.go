...
func (d *dialer) Dial() (_ transport.Pipe, err error) {
	...
	mrs := d.maxRecvSize
	d.lock.Unlock()
	p := transport.NewConnPipeSomeIP(conn, d.proto, nil)
...
func (l *listener) Listen() (err error) {
	...
	go func() {
		for {
			...
			mrs := l.maxRecvSize
			l.lock.Unlock()
			p := transport.NewConnPipeSomeIP(conn, l.proto, nil)
			p.SetMaxRecvSize(mrs)
...
type tcpSomeIPTran int

func (t tcpSomeIPTran) Scheme() string {
	return "tcp-someip"
}

func (t tcpSomeIPTran) NewDialer(addr string, sock mangos.Socket) (transport.Dialer, error) {
	var err error
...
func (t tcpSomeIPTran) NewListener(addr string, sock mangos.Socket) (transport.Listener, error) {
	var err error
	l := &listener{
		proto:      sock.Info(),
		closeq:     make(chan struct{}),
		handshaker: newConnHandshaker(),
	}
	...
	l.addr = addr
	l.handshaker = newConnHandshaker()
	return l, nil
}
...
func newConnHandshaker() transport.Handshaker {
	h := &handshaker{}
	h.cond = sync.NewCond(h)
	return h
}
